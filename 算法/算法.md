...
二分的本质是二段性，不是中间性，只要能够分成有区别的两段，就可以用

对于翻转数组，和第一个还是最后一个比较，主要是因为如果不旋转，就没有第一段，还是应该和最后一个比较才能找到问题

例如，[153. 寻找旋转排序数组中的最小值](https://leetcode.cn/problems/find-minimum-in-rotated-sorted-array/)

- 细节一：比较的时候带不带等于号
- 细节二：更新范围时，加不加一

- 区间的开闭，通常使用左闭右闭区间，主要是 right 的位置

- 设置 left = mid 的话，则会进入死循环

- while 中的条件，如果是 left <= ritght ，那么搜索结束后，left = right + 1，不存在搜索空间了。但是使用 left < right 的话，**终止条件是left = right,搜索空间是[right,right],此时该区间内非空**，**要使用这种条件，可以加上我们漏掉的搜索空间**

- ```C++
  return nums[left] == target ? left:-1;
  ```

- 只要使用左闭右闭区间，更新就要用 mid -1 或者 mid + 1
...
**双指针法**：首先进行排序，以便移动指针。然后固定一个 i 表示以 i 开头的三元组。left = i + 1，right 在末尾的位置，当三个位置的和相加**大于 0** 时，将 right 向左移动，**小于0**时，left向右移动。当**等于0**时，放到结果里，这时候，两边都需要移动，如果只移动一个，另一个不不动，那么结果仍然会重复。在移动的过程中注意重复。
...
遍历的顺序，是完全背包，背包从前向后遍历，至于 for 循环的嵌套顺序，因为本题目顺序是有影响的，如果先遍历物品再遍历背包，那么当第一个物品遍历完，后续再需要物品一的时候，就会找不到，因此需要**先遍历背包**

```C++
class Solution {
public:
    //物品就是单词，价值就是单词本身，背包就是字符串，物品可以随意使用，完全背包
    bool wordBreak(string s, vector<string>& wordDict) {
        //dp[i] 表示以 i-1 结尾的子串能否被拼接
        vector<bool> dp(s.size()+1, false);

        //初始化
        dp[0] = true;

        //for循环，先背包再物品，因为物品顺序有影响
        for(int j = 1; j < dp.size(); ++j){
            //遍历物品
            for(int i = 0; i < wordDict.size(); ++i){
                string value = wordDict[i];

                if(j < value.size())
                    continue;
                
                if(dp[j-value.size()] == true && 
                    strcmp(value.c_str(), s.substr(j-value.size(), value.size()).c_str()) == 0){
                    dp[j] = true;
                }
            }
        }
...
public:
    int maxProfit(vector<int>& prices) {
        ...
        for(int i = 1; i < prices.size(); ++i){
            //这天持有股票就是 前一天不持有，今天刚买，或者 前一天一直持有，今天不买765
...
另外要定义 dp[i] 代表 dp[i-1] 这样，是因为对于二维数组，如果不这样定义，需要把dp [0] [j] 和 dp[i] [0] 分别都初始化

以及是否需要变量保存，主要看以 i 结尾的时候，该位置是否参与到子序列的计算
...
public:
    //两个序列，子序列连续
    int findLength(vector<int>& nums1, vector<int>& nums2) {
        //dp[i][j] 表示 i-1,j-1 结尾的子数组（包括自身）公共的长度
...
这里不拆分 j 的原因是，当 j 小于 i 的一半的时候，拆分的是 i - j，但是当 j 超过一半时，计算的 dp [i-j] 相当于在拆分之前的 j，没啥区别
...
如果右边的比栈顶元素大，那么说明可以装雨水了，先把栈顶元素的**下标**保存下来，再**出栈**，然后计算现在的栈顶元素和当前遍历的值（一个当成左边的高度，一个当成右边的高度），计算接到雨水的高度：h = min(height[i], height[st.top()]) - height[mid]
...
最主要的思想就是，对一个区间内的元素找到中间位置，然后根据大小进行左右划分，当划分完成之后，返回中间的位置（该位置就是最终的位置）。然后对左右区间继续进行划分
...
对冒泡排序的改进，在内层遍历的时候，不是找到最小的就交换，而是遍历完，找到最小的位置再交换
...
前面的都是排好序的，从一个位置开始，向前插入，如果前面的比当前值大，继续向前找，同时把前面的元素向后移动
...
思路和快排很像，快排是先确定中间的位置，但是确定之后，左右仍是无序的，但是相对位置是确定的，然后对左右再找中间

归并排序也是先确定中间位置，不断划分，直到最后没法划分，然后合并，此时合并的左右两边都是有序的
...
private:
    void heapSort(vector<int>& vec){
        ...
        //此时，只是大根堆，并不是有序数组
        //将末尾元素与堆顶交换(这样最后一个数就是最大的了)，然后再调整
        for(int i = n - 1; i > 0; --i){
            swap(vec[0], vec[i]);
            //调整堆顶使arr[0...i - 1]再次成为大根堆
            adjust(vec, 0, i);
        }

    }
    
    void adjust(vector<int>& vec, int x, int n){
        //调整下标是 x 的元素
        int temp = vec[x];
        // 2*x + 1, 2*x + 2 分别是下标 x 的左右孩子
        //找到左右孩子里较大的一个，然后进行替换，替换之后，继续向下找
        for(int k = 2*x + 1; k < n; k = 2*k + 1){
            //如果右孩子比左孩子大
            if(k + 1 < n && vec[k+1] > vec[k]){
                k++;
            }
            //如果当前要调整的值比左右孩子小，就调整,较大值赋给x
            if(temp < vec[k]){
                vec[x] = vec[k];
                x = k;
            }
...
![](https://raw.githubusercontent.com/vaesong/Images/master/20230925094005.png)
...
现给出─组单词和行宽约束参数L，需要对文本进行处理，要求如下:1、规整后输出的每行所包含的字符个数应等于L。2、每行文本包含尽可能多的单词，单词之间或行尾不足部分都以“ * "填充。3、如果一行有两个或两个以上的单词，则要求第一个单词和最后一个单词分别与左右边界对齐。4、一行内单词间的“ *  "尽可能的均匀分布。如果无法均匀分布，则左边可多于右边1个“ * ”字符 5、最后一行的单词之间仅需一个“*”**输入描述**

单词列表。单词之间以","分隔 规整后输出的行宽约束参数L**输出描述**
...
class Solution{
public:
    vector<string> normStr(string& str, int N){
        deque<string> word;
        int start = 0, end = 0;
        ...
        while(end <= str.size()){
            ...
            if(str[end] == ','){
                word.push_back(string(str.begin() + start, str.begin() + end));
                start = end + 1;
                end++;
                
            }   
        }
        word.push_back(string(str.begin() + start, str.end()));

        while(!word.empty()){
            int len = 0;
            int num = 0;
            for(; num < word.size(); ++num){
                len += word[num].size();
                if(len >= N){
                    len -= word[num].size();
                    break;
            ...
            if(num >= 2){
                ...
                int n_ = 0;
                n_ = (N - len) / (num - 1);
                int mainder = (N - len) % (num - 1);
                for(int i = 0; i < (num-1); ++i){
                    if(mainder == 0)
                        star.push_back(string(n_, '*'));
                    else{
                        int remainder = mainder - i;
                        if(remainder != 0)
                            star.push_back(string(n_ + 1, '*'));
                        else
                            star.push_back(string(n_, '*'));
                    }
                }
...
class Solution{
public:
    int getCount(vector<int>& indegree, unordered_map<int, vector<int>>& G){
        //出队次数
        int res = 0;
        //出队个数
        int num = 0;
...
int main()
{
    ...
    unordered_map<int, vector<int>> G(N+1);
    ...
    Solution solu;
    cout << solu.getCount(indegree, G) << endl;
...
void del1(Node* dummyhead, int n){
    while(n-- > 0){
...
void del2(Node* dummyhead, int n){
...
void add3(Node* dummyhead, int n){
    //查看链表中是否存在资源
    Node* cur = dummyhead->next;
    while(cur != dummyhead){ 
        if(cur->val == n)
...
int main()
{
    ...
    while(dummyHead->next != dummyHead){
        Node* temp = dummyHead->next;
        dummyHead->next = temp->next;
        temp->next->pre = dummyHead;
        delete temp;
...
## [华为0506暑期实习笔试算法解析](https://mp.weixin.qq.com/s?__biz=MzkyNTQ3NDAzNw==&mid=2247485479&idx=1&sn=781c623e821a57b9a2563d6b90b81708&chksm=c1c745bdf6b0ccabe325b7bed4177addd5f4c1ac76feea003847e364ab4ca7b58eff9f4f5039&scene=126&sessionid=1683526572#rd)
...
3.输入数据保证 怪物位置、公主位置下，路障的状态循环一定为'000'，即路障一定不会出现
...
```
最快路王子移动顺序:(0,0)-> (0,0)-> (0,1)-> (0,2)-> (1,2)-> (2,2)
另一条路 (时间6) :
...
class node{
public:
    node(int x, int y, int time, int wait_time): x(x), y(y), time(time), wait_time(wait_time){}
public:
    int x;
    int y;
    int time;
...
int bfs(const vector<int>& pos, const vector<vector<bool>>& monster, const vector<vector<string>>& map, int N){
    //构建一个队列，用来存储可以到达的节点
    queue<node> Q;
    //定义一个访问数组
    vector<vector<bool>> visit(N, vector<bool>(N, false));
    Q.push(node(pos[0], pos[1], 0, 0));
    ...
    while(!Q.empty()){
        ...
        while(size-- > 0){
            ...
            //如果找到公主，直接返回
            if(cur.x == pos[2] && cur.y == pos[3])
                return cur.time;

            //然后向四周遍历
            for(int i = 0; i < 4; ++i){
                int dx = cur.x + dirs[i];
                int dy = cur.y + dirs[i+1];
                ...
                if(monster[dx][dy])
                    continue;
                //如果下一时刻路上有障碍
                if(map[dx][dy][(cur.time + 1)%3] == '1')
                    continue;

                //说明可以访问到该节点
                Q.push(node(dx, dy, cur.time+1, 0));
                visit[dx][dy] = true;
            }

            //如果下一个时间内没障碍，可以原地等待,但是最多等三次，三次是循环的轮数，如果隔壁还是不能通过，那就肯定不能等了
            if(cur.wait_time <= 2 && map[cur.x][cur.y][(cur.time + 1)%3] == '0'){
                Q.push(node(cur.x, cur.y, cur.time+1, cur.wait_time+1));
...
int main(){
    ...
    int K;
    cin >> K;
    vector<vector<bool>> monster(N, vector<bool>(N, false));
    for(int i = 0; i < K; ++i){
        int x = 0, y = 0;
        cin >> x >> y;
        monster[x][y] = true;
    ...
    for(int i = 0; i < N; ++i){
        ...
        for(int j = 0; j < str.size(); j += 4){
            string s = str.substr(j, 3);
            map[i].push_back(s);
        }
    }
    
    cout << bfs(pos, monster, map, N) << endl;
